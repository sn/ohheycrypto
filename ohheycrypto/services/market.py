...
class Market:
    def __init__(self):
        ...
        self.discord.setMarketValues(self.stop_loss, self.sell_threshold, self.buy_threshold, self.fiat, self.crypto)
    ...
    def _validate_config(self):
        ...
        if self.sell_threshold <= 0 or self.sell_threshold > 0.2:
            raise ValueError(f"Sell threshold must be between 0 and 0.2 (0-20%), got {self.sell_threshold}")
        
        if self.buy_threshold <= 0 or self.buy_threshold > 0.2:
            raise ValueError(f"Buy threshold must be between 0 and 0.2 (0-20%), got {self.buy_threshold}")
        
        if self.stop_loss <= self.sell_threshold:
            raise ValueError(f"Stop loss ({self.stop_loss}) must be greater than sell threshold ({self.sell_threshold})")
    ...
    def _should_buy_with_indicators(self, current_price: float, rsi: Optional[float], 
                                   trend: str, klines: List[float]) -> bool:
        """
    ...
    def _should_sell_with_indicators(self, current_price: float, rsi: Optional[float], 
                                    trend: str, last_order: Order) -> bool:
        """
    ...
    def calculate_position_size(self, current_price: float, fiat_balance: float) -> float:
        ...
        position_ratio = max_position_ratio - (normalized_volatility * (max_position_ratio - min_position_ratio))
    ...
    def buy(self) -> Order:
        ...
        if _fiat_balance < min_notional:
            raise ValueError(f"Insufficient balance: ${_fiat_balance:.2f} < ${min_notional:.2f} minimum")
        ...
        if _quantity * _price < min_notional:
            raise ValueError(f"Order value too small: ${_quantity * _price:.2f} < ${min_notional:.2f} minimum")

        # Reset trailing stop for new position
        self.last_high_price = None

        return Order(**self.client.create_order(
            symbol=self.symbol(),
    ...
    def sell(self) -> Order:
        ...
        if order_value < min_notional:
            raise ValueError(f"Order value too small: ${order_value:.2f} < ${min_notional:.2f} minimum")
            ...
            if last_order.is_sell():
                ...
                if float(self.price().get("askPrice")) < stop_loss_price:
                    ...
                    self.client.cancel_order(
                        symbol=self.symbol(), orderId=last_order.orderId
                    )
