...
class PositionSizing(BaseModel):
    ...
    min: float = Field(default=0.5, ge=0.1, le=1.0, description="Minimum position size as fraction of balance")
    max: float = Field(default=0.95, ge=0.1, le=1.0, description="Maximum position size as fraction of balance")
...
class TradingConfig(BaseModel):
    ...
    sell_threshold: float = Field(default=0.4, ge=0.1, le=5.0, description="Sell threshold percentage")
    buy_threshold: float = Field(default=0.2, ge=0.1, le=5.0, description="Buy threshold percentage")
    trailing_stop: float = Field(default=2.0, ge=0.5, le=10.0, description="Trailing stop percentage")
    position_sizing: PositionSizing = Field(default_factory=PositionSizing, description="Position sizing config")
...
class MarketAnalysisConfig(BaseModel):
    ...
    rsi_oversold: float = Field(default=30.0, ge=10.0, le=40.0, description="RSI oversold threshold")
    rsi_overbought: float = Field(default=70.0, ge=60.0, le=90.0, description="RSI overbought threshold")
    ...
    volatility_lookback: int = Field(default=20, ge=10, le=50, description="Volatility calculation lookback period")
...
class CircuitBreakerConfig(BaseModel):
    ...
    max_failures: int = Field(default=5, ge=3, le=10, description="Failed orders before circuit breaker")
    cooldown: int = Field(default=3600, ge=600, le=7200, description="Circuit breaker cooldown in seconds")


class ExecutionConfig(BaseModel):
    """Execution and timing configuration."""
    check_interval: int = Field(default=60, ge=30, le=300, description="Market check interval in seconds")
    circuit_breaker: CircuitBreakerConfig = Field(default_factory=CircuitBreakerConfig)
    min_order_value: float = Field(default=10.0, ge=5.0, le=50.0, description="Minimum order value in fiat")
    order_timeout: int = Field(default=30, ge=10, le=60, description="Order execution timeout in seconds")
