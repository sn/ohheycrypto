...
from ohheycrypto.database.models import Trade, BotState, MarketSnapshot, PerformanceMetrics, OrderSide
...
class Database:
    ...
    def save_trade(self, trade: Trade) -> int:
        ...
        with self.get_connection() as conn:
            ...
            """, (
                trade.order_id, trade.symbol, trade.side, trade.price,
                trade.quantity, trade.value, trade.fee, trade.fee_asset,
                trade.status, trade.timestamp, trade.rsi, trade.volatility,
                trade.trend, trade.buy_threshold, trade.sell_threshold,
                trade.stop_loss, trade.profit_loss, trade.profit_loss_percent,
    ...
    def save_market_snapshot(self, snapshot: MarketSnapshot):
        ...
        with self.get_connection() as conn:
            ...
            """, (
                snapshot.symbol, snapshot.price, snapshot.rsi,
                snapshot.volatility, snapshot.volume_24h,
                snapshot.price_change_24h, snapshot.ma_short,
                snapshot.ma_long, snapshot.trend, snapshot.timestamp
            ))
