...
class TradingBot:
    ...
    def analyze(self):
        ...
        for o in self.market.orders:
            if o["status"] == "NEW":
                LoggingService.success(
                    "OPEN: #{} - {} {} @ {:.4f} / ${:.2f}".format(
                        o["orderId"], o["side"].upper(), o["symbol"], 
        ...
        LoggingService.note("Current {} Price: ${:.2f}".format(self.market.symbol(), float(_p["lastPrice"])))
        LoggingService.note("High {} Price: ${:.2f}".format(self.market.symbol(), float(_p["highPrice"])))
        LoggingService.note("Low {} Price: ${:.2f}".format(self.market.symbol(), float(_p["lowPrice"])))
        ...
        while self.running:
            try:
                ...
                if self.market._is_circuit_breaker_active():
                    remaining_time = self.market.circuit_breaker_timeout - (time.time() - self.market.circuit_breaker_triggered_at)
                    LoggingService.warn(f"Circuit breaker active: {self.market.failed_orders_count} failures, {remaining_time/60:.1f}min remaining")
