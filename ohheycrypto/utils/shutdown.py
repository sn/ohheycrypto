...
class GracefulShutdown:
    ...
    def _perform_shutdown(self):
        ...
        for i, handler in enumerate(reversed(self.cleanup_handlers)):
            try:
                handler_name = getattr(handler, '__name__', f'handler_{len(self.cleanup_handlers) - i}')
...
class TradingBotShutdownManager:
    ...
    def setup_bot_cleanup(self, 
        ...
        if database:
            self.shutdown_handler.register_cleanup_handler(
                lambda: self._cleanup_database(database)
            )
    ...
    def _cleanup_market_service(self, market_service):
        ...
        try:
            ...
            if hasattr(market_service, 'circuit_breaker_active') and market_service.circuit_breaker_active:
                ...
                discord_service.post(
                    ...
                    payload={
                        ...
                        "embeds": [
                            {
                                "fields": [
                                    {"name": "Shutdown Time", "value": datetime.utcnow().strftime("%Y-%m-%d %H:%M:%S UTC")},
