...
class TestCLI:
    ...
    def test_version_command(self):
        ...
        import subprocess
        result = subprocess.run(['python', '-m', 'ohheycrypto.cli', '--version'], 
    ...
    def test_help_command(self):
        """Test --help command."""
        import subprocess
        result = subprocess.run(['python', '-m', 'ohheycrypto.cli', '--help'], 
                              capture_output=True, text=True, cwd=Path(__file__).parent.parent)
        assert result.returncode == 0
    ...
    def test_init_command(self):
        ...
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            import subprocess
            result = subprocess.run(['python', '-m', 'ohheycrypto.cli', 'init', str(config_path)], 
                                  capture_output=True, text=True, cwd=Path(__file__).parent.parent)
    ...
    def test_validate_command_invalid_config(self):
        ...
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            import subprocess
            result = subprocess.run(['python', '-m', 'ohheycrypto.cli', 'validate', str(config_path)], 
                                  capture_output=True, text=True, cwd=Path(__file__).parent.parent)
            
            # Should return non-zero exit code for invalid config
            assert result.returncode == 1
            assert 'API key not configured' in result.stdout or 'API key not configured' in result.stderr
    ...
    def test_run_command_help(self):
        ...
        import subprocess
        result = subprocess.run(['python', '-m', 'ohheycrypto.cli', 'run', '--help'], 
                              capture_output=True, text=True, cwd=Path(__file__).parent.parent)
        assert result.returncode == 0
